...
Component({
  properties: {
    ...
    proc_mode: {
      type: Number,
      value: 0 // Haiwei: 增加一个信息，用于指示从哪个表项过来
    },
    articleID: {
      type: String,
      value: '',
              ...
              (function (x) {
                p = p.then(function (data) {
                  ...
                  if (x == promiseFuncArr.length) {
                    ...
                    //Haiwei 
                    if(that.data.proc_mode == 0)
                    {
                      //Haiwei:这里将评论数写到分享表里面
                      var query = new AV.Query('Share');
                      query.get(that.data.articleID).then(function (cfg) {
                        // query.find().then(function (cfg) {
                        // 成功获得实例
                        console.log("Share comment ");
                        
                        cfg.set('comment_num', that.data.comment_num);
                        cfg.save();

                        console.log(that.data.comment_num);
                    ...
                    }else if(that.data.proc_mode ==1)
                    {
                      if (1)
                      {
                        //Haiwei:这里将评论数写到分享表里面
                        var service_query = new AV.Query('Service');
                        console.log(that.data.articleID);
                        service_query.get(that.data.articleID).then(function (Ser_ret) {
                          // query.find().then(function (cfg) {
                          // 成功获得实例
                          console.log("Service  comment ");
                         
                          Ser_ret.set('comment_num', that.data.comment_num);
                          console.log(that.data.comment_num);
                          Ser_ret.save();
                      ...
                      }else
                      {
                        ...
                        var service_query = AV.Object.extend('Service');
                        //beg
                        service_query.equalTo('objectId', that.data.articleID);
                        service_query.find()
                          .then(function (data) {
                            // 查询成功
                            console.log("Service  comment");
                            var num =data[0].get('comment_cnt');
                            console.log(num);
                            data[0].set('comment_cnt', that.data.comment_num);
                            //找到后修改标志(调试成功)
                            data[0].save();
