...
Page({
  ...
  getResponseFromServer: function () {
    ...
    {
      that.showPrompt('用户信息权限没有允许，请操作关于我们解决');
      return;
    }
    ...
    for (var i = 0; i < that.data.blackGrpid.length; ++i) {
      var tmp_query = new AV.Query('Service');
    ...
    //这里，如果地图设置一样，自己都很难看清楚
    var objectId = cur_marker.objectId;
    var root_query = new AV.Query('Service');
    root_query.equalTo('objectId', objectId);

    var sub_query = new AV.Query('Service');
    sub_query.equalTo('isRoot', objectId);
    //下面一定是or ，寻找和marer保存的objectid 相同，或者记录的父节点相同
    query = AV.Query.or(root_query, sub_query);
  ...
  commentItem: function (e) {
    ...
    wx.navigateTo({
      ...
      url: '../comment/comment?item=' + JSON.stringify(cur_item_id) + '&proc_mode='+ 1,
    })
  ...
  deleteSer: function (id) {
    var that = this;
    console.log(id);
    var m_item = that.data.servicelist[id];
    console.log(m_item);
    ...
    //下面查询：如果是子节点，应该就只找到一个。如果是根节点，会找到关联删除
  ...
  deleteProc: function (e) {
    ...
    console.log(id);
    console.log(that.data.servicelist[id]);
  ...
  onLoad: function (options) {
    ...
    //按照现在布局高度设置，如果发布主题内容太多，会导致图像压扁，这里限定主题内容
    if (cur_marker.content.length > 90 )
    {
      cur_marker.content = cur_marker.content.substring(0,90)+ " 。。。";
    }
    ...
    that.setData({
      ...
      cur_content:cur_marker.content
    })
